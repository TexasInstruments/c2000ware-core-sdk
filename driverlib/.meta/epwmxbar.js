    ...
    for (var muxConfigIndex in muxConfigs)
    {
        ...
        for (var muxesUsedIndex in muxesUsed)
        {
            ...
            if (usedMuxNumber == muxNumber)
            {
                ...
                if (inst["useSourceSelect"])
                {
                    inst[muxConfigName] = "XBAR_EPWM_MUX" + usedMuxNumberString + "_" + muxSource[usedMuxNumberString]
                }
                break;
            }
            else
...
for (var mux_number_index in device_driverlib_peripheral.XBAR_EPWMMuxConfig_All_MUXES)
{
    ...
    if (muxSpecificOptions[0] != undefined)
    {
        ...
        var noMuxNumOptions = muxSpecificOptions.map(element => {
            var elementName = element.name.replace(/XBAR_EPWM_MUX[0-9][0-9]_/g, "")
            var elementMux = element.name.substring("XBAR_EPWM_MUX".length, "XBAR_EPWM_MUX".length + 2)
