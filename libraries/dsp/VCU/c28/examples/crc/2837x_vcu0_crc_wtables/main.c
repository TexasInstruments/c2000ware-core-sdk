...
//#define CRC32_CRC32_MPEG2_TEST

#ifndef CRC32_CRC32_MPEG2_TEST

    #define CRC_TEST_PACKET_LEN		64

#else

    #define CRC_TEST_PACKET_LEN       4
    //use this to run CRC32, undefine this to run CRC32-MPEG2
    #define CRC32_ALGO
    uint16 testInput_flipped[CRC_TEST_PACKET_LEN];
#endif
...
//#pragma LOCATION(testInput,0x88000)
...
//	0xF0F1, 0xF2F3, 0xF4F5, 0xF6F7, 0xF8F9, 0xFAFB, 0xFCFD, 0xFEFF
    ...
    #pragma LOCATION(testInput,0x82000)
    static const uint16 testInput[CRC_TEST_PACKET_LEN] = {0x6BC1, 0xBEE2, 0x2E40, 0x9F96};
...
int main( void )
{
    ...
    crc16P2ResultC = getCRC16P2_cpu(INIT_CRC16, (uint16*)testInput,
    		(CRC_parity_e)CRC_parity_even,CRC_TEST_PACKET_BYTES);
    crc32ResultC   = getCRC32_cpu(0xFFFFFFFF, (uint16*)testInput,
    		(CRC_parity_e)CRC_parity_even,CRC_TEST_PACKET_BYTES);
    //*************************************************************************
    //! \endcode
    //!
...
#ifdef CRC32_CRC32_MPEG2_TEST
    #ifdef CRC32_ALGO
        // Bit reverse (16b) the input message buffer
        flipInputBuf_cpu((uint16*)testInput_flipped,(uint16*)testInput,CRC_TEST_PACKET_LEN);
        // Compute CRC32-MPEG2 with a seed value of 0xFFFFFFFF
        crc32ResultVcu   = getCRC32_vcu(0xFFFFFFFF, (uint16*)testInput_flipped,
                (CRC_parity_e)CRC_parity_even,CRC_TEST_PACKET_BYTES);
        // Bit reverse (32) the output CRC and logical NOT
        crc32ResultVcu = __flip32(crc32ResultVcu);
        crc32ResultVcu = ~crc32ResultVcu;
    #else
        // Compute CRC32-MPEG2
        crc32ResultVcu   = getCRC32_vcu_hilo_order_swap(0xFFFFFFFF, (uint16*)testInput,
                    (CRC_parity_e)CRC_parity_even,CRC_TEST_PACKET_BYTES);
    #endif
#endif
    //*************************************************************************
