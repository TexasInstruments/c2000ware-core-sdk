...
PrintWelcome(void)
{
    ...
    printf("Copyright (c) 2008-2011 Texas Instruments Incorporated. \
            All rights reserved.\n\n");
}
...
DownloadImage(tTIDFUHandle hHandle, tTIDFUParams *psDFU)
{
    ...
    //
    eRetcode = _TIDFUIsValidImage(hHandle, pcFileBuf, (unsigned long)iLen,
                                  &bTIFormat);
       ...
       bTIFormat)
    {
        ...
        eRetcode = _TIDFUDownload(hHandle, pcFileBuf, (unsigned long)iLen, 
                                  g_bSkipVerify, g_bDisregardIDs, NULL);
    }
    else
    {
        //
        ...
        if(eRetcode == DFU_OK)
        {
            ...
            else
            {
                ...
                if(iDevIndex == g_iDeviceIndex)
                {
                    ...
                    {
                        //
                        // If we were asked to switch modes and the device is 
                        // already in DFU mode, check that we were passed some 
                        // other operation to perform.
                        //
                        if(g_bSwitchMode)
                        {
