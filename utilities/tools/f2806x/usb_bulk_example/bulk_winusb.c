...
//****************************************************************************
static
DWORD GetDevicePath(LPGUID InterfaceGuid, PCHAR  pcDevicePath, 
                    size_t BufLen)
{
    BOOL bResult = FALSE;
    HDEVINFO deviceInfo;
    ...
    //
    bResult = SetupDiEnumDeviceInterfaces(deviceInfo, NULL, InterfaceGuid,
                                          0, &interfaceData);
    if(bResult == FALSE)
    {
        //
...
//****************************************************************************
BOOL
WriteUSBPacket(unsigned char *pcBuffer, unsigned long ulSize,
...
//****************************************************************************
BOOL
ReadUSBPacket(unsigned char *pcBuffer, unsigned long ulSize,
              unsigned long *pulRead)
{
    BOOL bResult;
    unsigned long ulSecondRead;

    bResult = WinUsb_ReadPipe(devInfo.winUSBHandle, devInfo.bulkInPipe,
                              pcBuffer, ulSize, pulRead, NULL);
    ...
    {
        bResult = WinUsb_ReadPipe(devInfo.winUSBHandle, devInfo.bulkInPipe,
                                  pcBuffer + *pulRead, ulSize - *pulRead,
                                  &ulSecondRead, NULL);
        if(bResult)
        {
